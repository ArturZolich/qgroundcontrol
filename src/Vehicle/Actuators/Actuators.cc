...
bool Actuators::parseJson(const QJsonDocument &json)
{
    ...
    for (const auto &&outputJson : outputs) {
        ...
        auto makeConditionFromValue = [this](const QJsonValue& obj, const char* key) {
            return Condition(obj[key].toString(""), _vehicle->parameterManager(), key);
        };
    ...
    auto makeConditionFromObject = [this](const QJsonObject& obj, const char* key) {
        return Condition(obj.value(key).toString(""), _vehicle->parameterManager(), key);
    };
    _showUi = makeConditionFromObject(obj, "show-ui-if");
